...
(function () {
  'use strict';
  const activeArea = document.getElementById('active-area'); // переменная для хранения области страницы, с которой работает local storage
  let otputArea; // переменная для хранения области страницы, куда выводятся задачи
  let toDoButton; // переменная для хранения элемента управления 'добавить задачу' (плюсик)
  let toDoText; // переменная для хранения input'а, куда пользователь вводит новую задачу
  let hideDoneButton; // переменная для хранения элемента управления 'скрыть/показать выполненные задачи' (глаз)
  let showDeletedButton; // переменная для хранения элемента управления 'перейти в корзину' (корзина)
  let hideDeletedButton; // переменная для хранения элемента управления 'выйти из корзины' (стрелка назад)
  ...
  const util = {
    ...
    closestParentElement : function(){}
  }
  
  var divOutputStart = '<div class="clearfix output"', // родительский div.output
    ...
    supports_storage = util.supportsHtml5Storage(); // проверяем есть ли поддержка Local Storage, записываем в переменную true если есть и false если нет
  ...
  document.getElementById('current-date').innerHTML = util.getDate(); // получаем текущую дату и записываем её в элемент с ID current-date
